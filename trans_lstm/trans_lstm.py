...
def trans_lstm(x_train, y_train, x_valid, y_valid, base_path, model_path):
    ...
    model_4 = tf.keras.Model(inputs=inputs,outputs=[output_1,output_2,output_3,output_4,output_5,output_6])
    model_4.summary()
    
    # transformer with lstm
    model_4.compile(loss={'o1':tf.keras.losses.MeanSquaredError(),
    ...
    history = model_4.fit(x=x_train,y=y_train,batch_size=32,epochs=25,validation_data=valid_data,callbacks=[earlystop, checkpoint, learning_rate_reduction])
    end_time = time.time()
    ...
    return model_4, training_time_per_epoch